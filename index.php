...
*/
		...
		echo $t->output('html/virus.html');
			...
			echo $t->output('html/nodata.html');
   	        ...
   	        $ntime = date('Y-m-d');
	        $sql = "select count(id) from $table2 where date like '%$ntime%' and content like '%发现新增可疑文件%' and source like '%line%'";
			$DB_site -> query($sql);
			$row = $DB_site -> fetch_array();
			$countkeywords = $row[0];
		$sqlkeyword = "select id,ip,content,source,level,remove,date from $table2 where date like '%$ntime%' and content like '%可疑文件%' and source like '%line%'";
...
}else if(trim($_GET['action']) == 'create'){
   	        $ntime = date('Y-m-d');
	        $sql = "select count(id) from $table2 where date like '%$ntime%' and content like '%发现新增可疑文件%' and source not like '%line%'";
			$DB_site -> query($sql);
			$row = $DB_site -> fetch_array();
			$countkeywords = $row[0];
		$sqlkeyword = "select id,ip,content,source,level,remove,date from $table2 where date like '%$ntime%' and content like '%可疑文件%' and source not like '%line%'";
		$DB_site -> query($sqlkeyword);
		while ($rowkeyword = $DB_site -> fetch_array()) {
				if ($rowkeyword['level']==1){
					$newlevel = "低";
		...
		$ntime = date('Y-m-d');
		$sql = "select count(id) from $table2 where date like '%$ntime%' and content like '%文件被修改%' and source like '%line%'";
		$DB_site -> query($sql);
			$row = $DB_site -> fetch_array();
			$countmodify = $row[0];
		$sqlcountmodify = "select id,ip,content,source,level,remove,date from $table2 where date like '%$ntime%' and content like '%文件被修改%' and source like '%line%'";
...
}else if(trim($_GET['action']) == 'modify'){
		$ntime = date('Y-m-d');
		$sql = "select count(id) from $table2 where date like '%$ntime%' and content like '%文件被修改%' and source not like '%line%'";
		$DB_site -> query($sql);
			$row = $DB_site -> fetch_array();
			$countmodify = $row[0];
		$sqlcountmodify = "select id,ip,content,source,level,remove,date from $table2 where date like '%$ntime%' and content like '%文件被修改%' and source not like '%line%'";
		$DB_site -> query($sqlcountmodify);
		while ($rowcountmodify = $DB_site -> fetch_array()) {
				if ($rowcountmodify['level']==1){
					$newlevel = "低";
...
}else if(trim($_GET['action'] == 'history')){
        echo $t->output('html/history.html');
}else if(isset($_POST[time])){
        $ntime = trim($_POST[time]);
        if(check_date($ntime)){
			$page = $_GET['page']?intval($_GET['page']):"1";
			$sql = "select id from $table2 where date like '%$ntime%'";
			mysql_query("set names 'utf-8'");
			$DB_site -> query($sql);
			$query = mysql_db_query($dbname, $sql);
		    $totail = $DB_site->num_rows();
			$number = $pagenumber; 
			$my_page = new PageClass($totail, $number, $_GET['page'], '?page2={page}');
			$sql = "SELECT * FROM $table2 where date like '%$ntime%' order by content DESC LIMIT " . $my_page -> page_limit . "," . $my_page -> myde_size;
			$DB_site -> query($sql);
			...
			$t->assign('all',$rows);
			$t->assign('page',$pag);
			echo $t->output('html/sort_date.html');
        }else{
                echo '日期格式不对，请重新输入.例:2013-07-29';
                echo $t->output('html/history.html');
        } 
